<template>
  ...
  <!-- #endif -->
  <img
    src="https://xcx-1251575231.cos.ap-shanghai.myqcloud.com/songzan/images/skeleton/index.jpg"
    alt=""
    v-if="!isLoad"
    class="loadingImg"
    mode="widthFix"
  ...
  <div class="index_ui">
      ...
      <div class="hotel-box">
        <div class="nav">
          ...
          <div
            ...
            @click="tabType(2)"
          >
            <div class="txt">酒店</div>
          </div>
          <img
            class="tab_bg"
            v-show="selectType == '2'"
            src="https://xcx-1251575231.cos.ap-shanghai.myqcloud.com/songzan/images/tabs_active_bg2.png"
          />
          <img
            class="tab_bg"
            v-show="selectType == '1'"
            src="https://xcx-1251575231.cos.ap-shanghai.myqcloud.com/songzan/images/tabs_active_bg.png"
          />
        </div>
        <div class="choose-box">
          <div class="choose-list">
            <div class="choose-item dropdown" @click="showDestination()">
              <div class="label">目的地</div>
              <div class="val-box">
                <div class="val">{{ dictionary.dictionaryDesc }}</div>
                <i class="iconfont icon-a-16_xialajiantou_ciji"></i>
              </div>
            </div>
            <div
              class="choose-item dropdown"
              @click="showFilterComponent()"
              v-if="selectType == 1"
            >
              <div class="label">住宿晚数</div>
              <div class="val-box">
                <div class="val small" v-for="(item, i) in days" :key="i">
                  <template v-if="item.date == dayNightDate">{{
                    item.name
                  }}</template>
                </div>
                <i class="iconfont icon-a-16_xialajiantou_ciji"></i>
            ...
            >
              ...
              <div class="timeBox">
                <div class="timeItem">{{ timeFilterMD(fromDate) }}</div>
                <div class="timeMiddle">住{{ zday }}晚</div>
                <div class="timeItem">{{ timeFilterMD(toDate) }}</div>
              </div>
            </div>
          </div>

          <div class="btn baseBtn" @click="toSearch()">查询</div>
        </div>
      </div>
    </div>
    <div class="notice">
      <notice ref="noticeRef" showLocation="0"></notice>
    </div>
    <div class="ad-banner" v-if="slides.left||slides.right">
      <div class="ad-swiper" v-if="slides.left && slides.left.length > 0">
        <swiper-box
          :slides="slides.left"
          :swiperConfig="adSwiperConfig"
        ></swiper-box>
      </div>
      <div class="ad-banner-rt" v-if="slides.right && slides.right.length > 0">
        <div
          class="ad-banner-img"
          v-for="(item, i) in slides.right"
          :key="i"
        ...
        >
          ...
          <div class="info" v-if="item.title || item.subTitle">
            ...
            <div class="subTitle" v-if="item.subTitle">
              {{ item.subTitle
              }}<i class="iconfont icon-a-12_youlajiantou_hui"></i>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- <div class="customized-product">
			<div class="product-img">
				<img class="img"
					src="https://website-10065452.cos.ap-shanghai.myqcloud.com/58fa7492-01ef-437f-a0a1-09754e8e06f1?imageView2/1/w/332/h/440">
			</div>
			<div class="product-info">
				<div class="product-name">松赞绝不止于旅行和酒店</div>
				<div class="product-desc">
					松赞可承揽公司团建、品牌活动、婚礼等，提供独一无二的场地和活动方案。
				</div>
				<div class="product-btn baseBtn">
					为我定制
				</div>
			</div>
...
import { reactive, ref, computed, onMounted, toRaw,nextTick } from "vue";
import api from "@/utils/api";

import { getStorage, removeStorage } from "@/utils/wxuser";
    ...
    if (res.result == 0) {
      ...
      if (res.retVal && res.retVal.datas && res.retVal.datas.length > 0) {
        ...
        if (datas.length > 0) {
          ...
          if(defType.value){
            typeList.forEach((n:any,j:number)=>{
               if( n.type==defType.value){
                 curType.value = n
                 curType.value.number = j
               }
            })
          }else{
          ...
          allTravelProduct(() => {
            ...
            if(defType.value){
              nextTick(()=>{
                tabsRef.value.clickTab(curType.value, curType.value.number)
              })
...
onMounted(() => {
  if(getStorage('type')){
    if(getStorage('type')==4){
      defType.value = 'FreeTravel'
    }else{
      defType.value = getStorage('type')
    }
